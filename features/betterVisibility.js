...
defaultSettings.onClick((item, slotNumber) => {
    if (slotNumber < defaultSettings.getNumberOfSlots()) {
        ...
        if (itemname === "§5Custom Visibility") {
            ...
            if (!split[0].includes("Visibility")) {return}
...
register("chat", (value, e) => {
    if (!Settings.betterVisibility) {return;}
    cancel(e);
...
register("messageSent", (msg, e) => {
    if (!Settings.betterVisibility) {return;}
    if (!inputtingCustomValue) {return;}
...
register("command", (arg) => {
    ...
    defaultSettings.open(); 
}).setName("visibility").setAliases(["visiblity", "visi", "visible"]);

register("tick", () => { if (inHouse === true) {setupDefaultSettingsItems()} });