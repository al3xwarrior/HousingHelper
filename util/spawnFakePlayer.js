...
export function createFakePlayer(xpos, ypos, zpos, pitch, yaw, name, skinURL) {
    let skinType = 'slim';
    const mojangProfile = JSON.parse(FileLib.getUrlContent('https://sessionserver.mojang.com/session/minecraft/profile/42986ec349454aa0852a1ec92a48cf99'));
    ...
    const textureObject = new ThreadDownloadImageData(null, skinURL, null, new ImageBufferDownload());
    textureManager.func_110579_a(resourceLocation, textureObject); // loadTexture

    const profile = new com.mojang.authlib.GameProfile(java.util.UUID.randomUUID(), name);

    const otherPlayer = new EntityOtherPlayerMP(World.getWorld(), profile);

    const playerInfo = new NetworkPlayerInfo(profile);
    ReflectionHelper.setPrivateValue(net.minecraft.client.network.NetworkPlayerInfo, playerInfo, resourceLocation, 'locationSkin', 'field_178865_e');

    ReflectionHelper.setPrivateValue(net.minecraft.client.network.NetworkPlayerInfo, playerInfo, skinType, 'skinType', 'field_178863_g');

    ReflectionHelper.setPrivateValue(net.minecraft.client.entity.AbstractClientPlayer, otherPlayer, playerInfo, 'playerInfo', 'field_175157_a');
